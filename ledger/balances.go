...
func (b *Balances) Validate(tx Transaction) error {
	...
	// Rule 2: recipient must not be empty — money cannot be burned to nowhere.
	if tx.Recipient == "" {
		return fmt.Errorf("invalid transaction: recipient cannot be empty")
	}
	// Rule 3: reject overdrafts — you can't spend money you don't have.
	// The faucet (empty Sender) is exempt because it creates money, not transfers it.
	// However, we cap the faucet to a maximum of 100 coins per mint to prevent infinite money.
	if tx.Sender == "" {
		if tx.Amount > 100 {
			return fmt.Errorf("invalid transaction: faucet mint amount %d exceeds maximum of 100", tx.Amount)
		}
	} else if b.accounts[tx.Sender] < tx.Amount {
